...
const ListingDetail = async ({ searchParams }: any) => {
  const nftCollectionAddress = searchParams?.nftCollectionAddress;
  const listingId = searchParams?.listingId;

  const data = await getMarketplaceItem({ nftCollectionAddress, listingId });
  if (data.message) {
    redirect("/login");
  }
  const nft = data.data;
  ...
  return (
    <>
      <Head>
        <meta
          property="og:url"
          content={`${BASE_UI_URL}/nft/collection/detail/?nftCollectionAddress=${nftCollectionAddress}&listingId=${listingId}`}
        />
        ...
        <meta
          ...
          content={`${BASE_UI_URL}/nft/collection/detail/?nftCollectionAddress=${nft?.nftCollectionAddress}&listingId=${listingId}`}
        />
